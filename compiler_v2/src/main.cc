...
bool is_c(char x) { return (x >= 'a' && x <= 'z') || (x >= 'A' && x <= 'Z'); }
bool is_n(char x, ParserState s) {
  return x == '.' ? s == READ_NUM : (x >= '0' && x <= '9');
}
bool is_o(std::string x) {
  return std::find(OPS.begin(), OPS.end(), x) != OPS.end();
}
bool is_w(char x) { return x == '\r' || x == '\n' || x == '\t' || x == ' '; }

ParserState grab_state(char x, ParserState s) {
  if (x == '\"') {
    return READ_STRING;
  }
  if (is_c(x)) {
...
std::string clear_buf(std::vector<char> &b) {
