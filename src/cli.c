...
void dyn_cli_help(const char *command) {
  ...
  printf(
      ...
      "  --diagnostics json  --check (fmt only)  --json (docs only)");
}

int dyn_cli_parse(int argc, char **argv, DynOptions *o) {
  memset(o, 0, sizeof(*o));
  o->debug_info = true;
  o->target = DYN_HOST_TARGET;
  if (argc < 2)
    return 2;
  o->command = argv[1];
  for (int i = 2; i < argc; ++i) {
    ...
    if (strcmp(a, "--output") == 0 || strcmp(a, "--target") == 0 ||
      ...
      } else if (strcmp(a, "--max-work") == 0) {
        char *end = NULL;
        errno = 0;
        unsigned long long value = strtoull(argv[i], &end, 10);
        if (errno || argv[i][0] < '0' || argv[i][0] > '9' || *end || !value) {
          fprintf(stderr, "error: --max-work requires a positive integer\n"); return 2;
        }
        o->max_work = value;
  ...
  if (o->docs_json && strcmp(o->command, "docs")) {
    fprintf(stderr, "error: --json is only supported by docs\n");
    return 2;
  }
  return 0;
}
