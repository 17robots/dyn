...
static int sources_load(const DynContext *context, const char *dir,
  ...
  while ((entry = readdir(d))) {
    ...
    s->text[s->length] = 0;
    bool failed = s->length != (size_t)size || ferror(f);
    if (fclose(f))
      failed = true;
...
/* Target filtering precedes parsing, including for files excluded from this
   target. Ignore comment and literal contents without requiring an otherwise
   valid AST. */
static const char *source_skip_trivia(const char *p) {
  for (;;) {
...
static const char *source_target_directive(const char *p) {
  while (*(p = source_skip_trivia(p))) {
    if (*p == '\"' || *p == '\'') {
      char quote = *p++;
      while (*p && *p != quote) {
        if (*p == '\\' && p[1])
...
int dyn_source_target_enabled(const DynSource *s) {
  /* Most files have no target directive. The substring test is only a
     rejection fast path; the lexer below still excludes comments/literals. */
  if (!strstr(s->text, "#target")) return 1;
  const char *p = source_target_directive(s->text);
  if (!p)
    return 1;
  ...
  while (*p && *p != ')') {
    p = source_skip_trivia(p);
    if (*p == ',')
      p = source_skip_trivia(p + 1);
    char key[32] = {0}, value[32] = {0};
    size_t k = 0, v = 0;
    while ((*p == '_' || isalnum((unsigned char)*p)) && k + 1 < sizeof(key))
      key[k++] = *p++;
    p = source_skip_trivia(p);
    if (*p++ != ':')
      return -1;
    p = source_skip_trivia(p);
    while ((*p == '_' || isalnum((unsigned char)*p)) && v + 1 < sizeof(value))
      value[v++] = *p++;
    const char *want =
        dyn_target_property(dyn_context_target(&s->context), key);
    if (!want)
      return -1;
    if (strcmp(value, want))
      return 0;
    p = source_skip_trivia(p);
  }
  return *p == ')' ? 1 : -1;
...
int dyn_sources_merge(const DynSources *sources, const char *module_name,
  ...
  for (size_t i = 0; i < sources->count; ++i) {
    ...
    if (!reflection && strstr(sources->items[i].text, "#typeof")) {
      ...
      if (!tree)
        return 2;
      reflection = dyn_syntax_has_reflection(ts_tree_root_node(tree));
      ts_tree_delete(tree);
    }
  ...
  for (size_t i = 0; i < sources->count; ++i) {
    if (!dyn_source_target_enabled(&sources->items[i]))
      continue;
    const DynSource *input = &sources->items[i];
    DynSourceMap *map = &out->maps[out->map_count++];
    map->path = strdup(input->path);
    map->original_text =
        strdup(input->original_text ? input->original_text : input->text);
    map->original_length =
        input->original_text ? input->original_length : input->length;
    if (!map->path || !map->original_text) {
      dyn_source_free(out);
      return 2;
    }
    map->start = at;
  ...
  for (size_t i = 0, map = 0; i < sources->count; ++i) {
    ...
    if (input->syntax && input->length >= 4096 &&
        (!seed || input->length > seed->length)) {
      seed = input; seed_start = out->maps[map].start;
    }
    ++map;
  }
  if (seed) {
    TSTree *tree = ts_tree_copy(seed->syntax);
    TSPoint end = dyn_syntax_advance((TSPoint){0}, seed->text, seed->length);
    size_t suffix = seed_start + seed->length;
    TSInputEdit append = {(uint32_t)seed->length, (uint32_t)seed->length,
        (uint32_t)(at - seed_start), end, end,
        dyn_syntax_advance(end, out->text + suffix, at - suffix)};
    ts_tree_edit(tree, &append);
    TSInputEdit prepend = {0, 0, (uint32_t)seed_start, {0}, {0},
        dyn_syntax_advance((TSPoint){0}, out->text, seed_start)};
    ts_tree_edit(tree, &prepend);
    out->syntax = dyn_syntax_reparse(out->text, out->length, tree);
    ts_tree_delete(tree);
    if (!out->syntax) { dyn_source_free(out); return 2; }
  }
  return 0;
}
void dyn_source_position(const DynSource *s, size_t byte, const char **path,
                         const char **original, size_t *original_length, size_t *original_offset) {
  const char *text = s->original_text ? s->original_text : s->text;
  size_t length = s->original_text ? s->original_length : s->length,
         offset = byte;
  *path = s->path;
  for (size_t i = 0; i < s->map_count; ++i)
    if (byte >= s->maps[i].start && byte <= s->maps[i].end) {
      const DynSourceMap *m = &s->maps[i];
      size_t local = byte - m->start;
      *path = m->path;
      text = m->original_text;
      length = m->original_length;
      offset = local;
      for (size_t j = 0; j < m->span_count; ++j)
        if (local >= m->spans[j].generated_start &&
            local <= m->spans[j].generated_end) {
          DynSourceSpan p = m->spans[j];
          size_t gn = p.generated_end - p.generated_start,
                 on = p.original_end - p.original_start;
          offset = p.original_start +
                   (gn ? (local - p.generated_start) * on / gn : 0);
          break;
        }
      break;
    }
  if (!s->map_count && s->span_count)
    for (size_t j = 0; j < s->span_count; ++j)
      if (byte >= s->spans[j].generated_start &&
          byte <= s->spans[j].generated_end) {
        DynSourceSpan p = s->spans[j];
        size_t gn = p.generated_end - p.generated_start,
               on = p.original_end - p.original_start;
        offset =
            p.original_start + (gn ? (byte - p.generated_start) * on / gn : 0);
        break;
      }
  *original = text; *original_length = length; *original_offset = offset;
}
...
void dyn_source_free(DynSource *s) {
  ...
  free(s->spans);
  free(s->dependency_sources);
  for (size_t i = 0; i < s->map_count; ++i) {
    free(s->maps[i].path);
    free(s->maps[i].original_text);
    free(s->maps[i].spans);
  }
  free(s->maps);
  memset(s, 0, sizeof(*s));
