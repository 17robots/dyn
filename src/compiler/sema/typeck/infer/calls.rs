...
pub(super) fn infer_call_type(
    ...
    for arg in &call.args {
        if let Some(name) = &arg.name {
            ...
            if sig.params[param_index].comp && !is_compile_time_expr(&arg.value) {
                diagnostics.push(
                    Diagnostic::error(
                        DiagnosticPhase::TypeChecker,
                        DiagnosticCode::E4005,
            ...
            if sig.params[positional_count].comp && !is_compile_time_expr(&arg.value) {
                diagnostics.push(
                    Diagnostic::error(
                        DiagnosticPhase::TypeChecker,
                        DiagnosticCode::E4005,
    ...
    match &expr.kind {
        ...
        ExprKind::TypeConstruct { ty_expr, fields } => {
            contains_disallowed_inline_flow(ty_expr)
                || fields.iter().any(|field| contains_disallowed_inline_flow(&field.value))
        }
