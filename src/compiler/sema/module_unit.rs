...
use crate::compiler::module_resolver::{ModuleGraph, ModuleId, ModuleKey};
...
pub fn build_module_units(graph: &ModuleGraph, parsed: &ParseSession) -> Vec<ModuleUnit> {
    ...
    for parsed_file in &parsed.files {
        ...
        let absolute_path = graph.root_dir.join(&parsed_file.file_path);
...
mod tests {
    ...
    fn merges_declarations_by_module_id() {
        ...
        assert!(main
            .declarations
            .iter()
            .any(|decl| decl.file_path == PathBuf::from("a.dyn")));
        assert!(main
            .declarations
            .iter()
            .any(|decl| decl.file_path == PathBuf::from("b.dyn")));
