...
static uint64_t hash_bytes(uint64_t hash, const void *data, size_t length) {
  if (!hash)
    return 0; /* Zero is reserved for an unavailable cache key. */
...
bool dyn_cache_directory(char *path, size_t capacity) {
  ...
  if (configured && *configured)
    ...
    n = snprintf(path, capacity, "%s/dyn/c-frontend-" DYN_FRONTEND_VERSION, xdg);
  else if (home && *home)
...
static int cache_walk(const char *path, bool remove_files, uint64_t *bytes,
  ...
  while (!result && (entry = readdir(directory))) {
    ...
    if (S_ISDIR(st.st_mode)) {
      result = cache_walk(child, remove_files, bytes, files);
      if (remove_files && !result && rmdir(child))
        result = 2;
    } else {
      *bytes += (uint64_t)st.st_size;
      ++*files;
      if (remove_files && unlink(child))
        result = 2;
    }
  }
  closedir(directory);
...
int dyn_cache_command(const char *action) {
  ...
  if (!strcmp(action, "clean")) {
    ...
    if (!result)
      printf("removed %llu cache files (%llu bytes)\n",
             (unsigned long long)files, (unsigned long long)bytes);
    return result;
  }
...
static uint64_t hash_project_sources(uint64_t hash, const DynSources *sources) {
  /* Fingerprinting is short CPU work. Forking a build worker for each module
     costs more than hashing its bytes. Preserve the existing grouped key. */
  for (size_t first = 0; first < sources->count;) {
    const char *path = sources->items[first].path, *slash = strrchr(path, '/');
    size_t length = slash ? (size_t)(slash - path) : 0, end = first + 1;
    while (end < sources->count) {
      const char *next = sources->items[end].path, *last = strrchr(next, '/');
      size_t size = last ? (size_t)(last - next) : 0;
      if (size != length || memcmp(path, next, length)) break;
      ++end;
    }
    uint64_t module = 0;
    hash_module(sources, first, end - first, NULL, &module);
    hash = hash_bytes(hash, &module, sizeof(module));
    first = end;
  }
  return hash;
}
...
static bool copy_file(const char *from, const char *to) {
  ...
  do {
    ...
    if (ferror(input) || fwrite(bytes, 1, count, output) != count) {
      ok = false;
      break;
    }
  } while (count == sizeof(bytes));
...
static bool fast_stat(FILE *file, const char *path, bool write) {
  ...
  if (stat(path, &st) || length > UINT32_MAX)
    return false;
  ...
  FastEntry stored;
  char *name;
  if (fread(&stored, sizeof(stored), 1, file) != 1 || stored.path_length > 4096)
    return false;
  ...
  ok = ok && !strcmp(name, path) && stored.device == entry.device &&
       ...
       stored.seconds == entry.seconds &&
       stored.nanoseconds == entry.nanoseconds;
...
bool dyn_cache_fast_hit(const DynOptions *options, const char *compiler_path,
  ...
  if (ok)
    ok = fread(&source_count, sizeof(source_count), 1, file) == 1 &&
         source_count <= 16384;
  for (uint32_t i = 0; ok && i < source_count; ++i) {
    FastEntry stored;
    if (fread(&stored, sizeof(stored), 1, file) != 1 ||
        stored.path_length > 4096) {
      ok = false;
      break;
    }
    char *name = malloc((size_t)stored.path_length + 1);
    if (!name) {
      ok = false;
      break;
    }
    ok = fread(name, 1, stored.path_length, file) == stored.path_length;
    name[stored.path_length] = 0;
    struct stat st;
    ok = ok && !stat(name, &st) && stored.device == (uint64_t)st.st_dev &&
         stored.inode == (uint64_t)st.st_ino &&
         stored.size == (uint64_t)st.st_size &&
         stored.seconds == (uint64_t)st.st_mtim.tv_sec &&
         stored.nanoseconds == (uint64_t)st.st_mtim.tv_nsec &&
...
static void fast_store(const DynSources *sources, const DynOptions *options,
  ...
  for (size_t i = 0; ok && i < options->link_input_count; ++i)
    ok = fast_stat(file, options->link_inputs[i], true);
  long count_position = file ? ftell(file) : -1;
  uint32_t count = 0;
  if (count_position < 0)
    ok = false;
  if (ok)
    ok = fwrite(&count, sizeof(count), 1, file) == 1;
  for (size_t i = 0; ok && i < sources->count; ++i) {
    ok = fast_stat(file, sources->items[i].path, true);
    if (ok)
      ++count;
    char directory[4096];
    size_t length = strlen(sources->items[i].path);
    if (length >= sizeof(directory)) {
      ok = false;
      break;
    }
    memcpy(directory, sources->items[i].path, length + 1);
    char *slash = strrchr(directory, '/');
    if (slash) {
      *slash = 0;
      bool seen = false;
      for (size_t j = 0; j < i; ++j) {
        const char *prior = sources->items[j].path;
        size_t n = (size_t)(strrchr(prior, '/') - prior);
        if (strlen(directory) == n && !memcmp(directory, prior, n)) {
          seen = true;
          break;
        }
      }
      if (!seen) {
        ok = fast_stat(file, directory, true);
        if (ok)
          ++count;
        char manifest[4096];
        if (snprintf(manifest, sizeof(manifest), "%s/dyn.project", directory) <
                (int)sizeof(manifest) &&
            !access(manifest, F_OK)) {
          ok = fast_stat(file, manifest, true);
          if (ok)
            ++count;
        }
      }
  ...
  if (ok) {
    long end = ftell(file);
    ok = fseek(file, count_position, SEEK_SET) == 0 &&
...
bool dyn_cache_hit(const DynSources *sources, const DynOptions *options,
  ...
  bool hit =
      file && fscanf(file, "%llx %llx", &stored, &artifact) == 2 && stored &&
      artifact &&
      (uint64_t)stored == build_key(sources, options, compiler_path) &&
      (uint64_t)artifact == hash_file(UINT64_C(1469598103934665603), output);
  if (file)
    fclose(file);
  free(path);
  return hit;
...
void dyn_cache_store(const DynSources *sources, const DynOptions *options,
  ...
  if (file) {
    fprintf(
        ...
        (unsigned long long)hash_file(UINT64_C(1469598103934665603), output));
    fclose(file);
  }
  free(path);
...
bool dyn_object_cache_restore(const DynSources *sources,
  ...
  bool hit =
      artifact && file && fscanf(file, "%llx %llx", &stored, &digest) == 2 &&
      stored && digest &&
...
static uint64_t module_key(const DynSources *sources, size_t first,
  ...
  bool bitcode =
      object_length >= 3 && !strcmp(object + object_length - 3, ".bc");
  hash = hash_bytes(hash, &bitcode, sizeof(bitcode));
  /* Dependency graph and syntax-based declaration hashes are captured during
     rewriting. Sources without that metadata use a conservative content key. */
  uint64_t dependencies = sources->items[first].dependency_hash;
  if (!dependencies) dependencies = hash_project_sources(0, sources);
  hash = hash_bytes(hash, &dependencies, sizeof(dependencies));
  uint64_t value = 0;
  if (hash_module(sources, first, count, NULL, &value))
    return 0;
...
static bool module_artifact(char *path, size_t capacity, const char *root,
  ...
  if (mkdir(root, 0777) && errno != EEXIST)
    return false;
  int n =
      snprintf(path, capacity, "%s/%016llx.o", root, (unsigned long long)key);
...
bool dyn_module_cache_restore(const DynSources *sources, size_t first,
  ...
  bool valid =
      file && fscanf(file, "%llx", &digest) == 1 && digest &&
      (uint64_t)digest == hash_file(UINT64_C(1469598103934665603), artifact);
  if (file)
    fclose(file);
