...
pub const Checker = struct {
    ...
    fn checkStatement(s: *Checker, file_id: u32, n: ast.Node) void {
        switch (n.type) {
            .block => |b| {},
            .declaration => {},
            .if_statement => |if_| {},
            .for_statement => |for_| {},
            .while_statement => |while_| {},
            .match_statement => |match_| {},
            .return_expression => |return_| {},
            .break_expression => |break_| {},
        }
    }
    fn declareCapture(s: *Checker, file_id: u32, cap: ast.Node) void {
        for (cap.type.capture.captures.items) |c| {
            const item = c.type.capture_val;
            const name = item.val.type.identifier;
            s.scopes.declare(.{ .name = name, .kind = .@"var", .span = c.span, .mutability = if (item.mut) .mutable else .immutable }, s.diags, file_id);
        }
    }
};
