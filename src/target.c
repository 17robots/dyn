...
static const DynTarget targets[] = {{
    ...
    "/lib64/ld-linux-x86-64.so.2", "x86_64", "linux", "sysv", "none",
    "little", "64", true},
...
const char *dyn_target_property(const DynTarget *target, const char *key) {
  if (!strcmp(key, "arch")) return target->arch;
  if (!strcmp(key, "kernel")) return target->kernel;
  if (!strcmp(key, "abi")) return target->abi;
  if (!strcmp(key, "libc")) return target->libc;
  if (!strcmp(key, "endian")) return target->endian;
  if (!strcmp(key, "pointer_bits")) return target->pointer_bits;
  return NULL;
}

bool dyn_target_syscall_number(uint64_t source, uint64_t *native) {
  if (strcmp(dyn_target->kernel, "linux")) return false;
  if (!strcmp(dyn_target->arch, "x86_64")) { *native = source; return true; }
  ...
  static const struct { uint16_t portable, aarch64; } map[] = {
    {0,63},{1,64},{3,57},{8,62},{9,222},{11,215},{13,134},{16,29},
    {24,124},{32,23},{33,24},{35,101},{39,172},{41,198},{42,203},
    {43,202},{44,206},{45,207},{48,210},
    {49,200},{50,201},{51,204},{52,205},{54,208},{59,221},{60,93},
  ...
  for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); ++i)
    if (map[i].portable == source) { *native = map[i].aarch64; return true; }
  return false;
}
