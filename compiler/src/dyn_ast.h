...
/* Append-only declaration index. Spans borrow the AST's source revision. */
typedef struct {
  uint32_t *buckets, *next;
  size_t count, capacity;
} DynNameIndex;
uint32_t dyn_name_index_find(DynNameIndex *, const void *, size_t, size_t,
                            DynSpan, const DynSource *, bool, bool *);
...
bool dyn_ast_lower_main(TSNode function, const DynSource *source,
                        DynAstProgram *ast, unsigned *errors);
bool dyn_ast_parse_main_source(const DynSource *source, DynAstProgram *ast,
                               unsigned *errors);
/* Parse every declaration, but lower bodies only for owner_key. NULL lowers
   all. This is the frontend seam used by per-module compilation and cached
   interfaces: dependencies contribute types/signatures, never bodies. */
bool dyn_ast_parse_source_owner(const DynSource *source, DynAstProgram *ast,
...
void dyn_type_format(DynAstProgram *, DynType, const DynSource *, char *,
                     size_t);
