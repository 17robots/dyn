...
static DynType lower_type(const DynAstProgram *a, DynType t) {
  size_t guard = 0;
...
bool dyn_ir_lower(const DynAstProgram *a, const DynSource *source,
                  DynIrProgram *ir) {
  ...
  for (size_t i = 0; i < a->field_count; ++i) {
    ir->fields[i] =
        (DynIrField){.name = copy_span(a->fields[i].name, source),
  ...
  for (size_t i = 0; i < a->struct_count; ++i) {
    ir->structs[i] =
        (DynIrStruct){.name = copy_span(a->structs[i].name, source),
                      .span = a->structs[i].span,
                      .packed = a->structs[i].packed,
                      .is_public = a->structs[i].is_public,
                      .module_owner = a->structs[i].module_owner,
                      .field_start = a->structs[i].field_start,
                      .field_count = a->structs[i].field_count,
                      .size = a->structs[i].size,
                      .alignment = a->structs[i].alignment};
    if (!ir->structs[i].name) {
      dyn_ir_free(ir);
      return false;
    }
  }
  for (size_t i = 0; i < a->enum_count; ++i) {
    ir->enums[i] =
        (DynIrEnum){.name = copy_span(a->enums[i].name, source),
                    .span = a->enums[i].span,
                    .tag_type = a->enums[i].tag_type,
                    .variant_start = a->enums[i].variant_start,
                    .variant_count = a->enums[i].variant_count,
                    .size = a->enums[i].size,
  ...
  for (size_t i = 0; i < a->variant_count; ++i) {
    ir->variants[i] = (DynIrVariant){
        .name = copy_span(a->variants[i].name, source),
        .payload_type = lower_type(a, a->variants[i].payload_type),
        .tag = a->variants[i].tag};
    if (!ir->variants[i].name) {
      dyn_ir_free(ir);
      return false;
    }
  }
  ...
  for (size_t i = 0; i < a->fn_type_count; ++i)
    ir->fn_types[i] = (DynIrFnType){
        lower_type(a, a->fn_types[i].return_type), a->fn_types[i].param_start,
        a->fn_types[i].param_count, a->fn_types[i].variadic};
  ...
  for (size_t i = 0; i < a->statement_count; ++i)
    ir->statements[i] = (DynIrStmt){a->statements[i].kind,
                                    a->statements[i].expression,
                                    a->statements[i].target,
                                    a->statements[i].assignment_op,
                                    a->statements[i].local_id,
                                    a->statements[i].body_start,
                                    a->statements[i].body_count,
                                    a->statements[i].else_start,
                                    a->statements[i].else_count,
                                    a->statements[i].loop_id,
                                    a->statements[i].target_loop_id,
                                    a->statements[i].case_arm_start,
                                    a->statements[i].case_arm_count,
  ...
  for (size_t i = 0; i < a->pattern_count; ++i)
    ir->patterns[i] = (DynIrPattern){a->patterns[i].first_value,
                                     a->patterns[i].last_value,
                                     lower_type(a, a->patterns[i].type),
                                     a->patterns[i].variant,
                                     a->patterns[i].is_enum,
...
void dyn_ir_free(DynIrProgram *ir) {
  ...
  for (size_t i = 0; ir->params && i < ir->param_count; ++i)
    free(ir->params[i].name);
  for (size_t i = 0; ir->locals && i < ir->local_count; ++i)
    free(ir->locals[i].name);
  for (size_t i = 0; ir->fields && i < ir->field_count; ++i)
    free(ir->fields[i].name);
  for (size_t i = 0; ir->structs && i < ir->struct_count; ++i)
    free(ir->structs[i].name);
  for (size_t i = 0; ir->enums && i < ir->enum_count; ++i)
    free(ir->enums[i].name);
  for (size_t i = 0; ir->variants && i < ir->variant_count; ++i)
    free(ir->variants[i].name);
  free(ir->expressions);
  free(ir->items);
  free(ir->fields);
  free(ir->structs);
